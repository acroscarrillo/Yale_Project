...
ϵ_1_max = 14 #MHz
ϵ_2_max = 18 #MHz
K_exp = 0.505 #MHz not used anywhere, for reference only 
...
K = (10*g_n[1]^2)/3 - 3*g_n[2]/2
Ω_1_array = Vector( range(0, 2*(ϵ_1_max), length=150) )
...
# Define data form
data_array = zeros( (N-1)*length(Ω_1_array)*length(Ω_2_array), 6 ) # data form: "ϵ_n","Δnn","n","ϵ_2","ϵ_1","K"]
...
for (i,_) in enumerate(Ω_1_array)
    for (j,_) in enumerate(Ω_2_array)
        ...
        # Π_temp = 2*(Ω_2*ω_2)/(ω_2^2-ω_0^2)
        # ϵ_1, ϵ_2 = 2*Ω_1, g_n[1]*Π_temp
        ϵ_1, ϵ_2 = 2*Ω_1, g_n[1]*Π(Ω_2,ω_2)
        ...
        for n=1:(N-1)
            ...
            data_array[counter,:] .= ϵ_n[n+1], Δnn_array[n], n, ϵ_2, ϵ_1, K
            counter += 1
        end
...
df_floquet = DataFrame(data_array, ["ϵ_n","Δnn","n","ϵ_2","ϵ_1","K"]) 
...
# heatmap(ϵ_1_array[2:end],ϵ_2_array,log.(matrix_color[2:end,:]'), xlab=L"\epsilon_1/\omega_0",ylab=L"\epsilon_2/\omega_0",title=L"N="*string(N))
