...
K_array = Vector( range(0, K_max, length=150) )
...
# Define data form
data_array = zeros( (N-1)*length(K_array)*length(Ω_2_array), 12 ) # data form: ϵ_n | Δnn | n | ω_0 | Ω_1 | ω_1 | Ω_2 | ω_2 | K | ϵ_2 | ϵ_1 | N 

# The following is used to find ϵ_0 as described in the paper for your eyes only
E_0 , V_0_array = eigen(H_0(N,ω_0,g_n))


matrix_color = zeros(length(K_array),length(Ω_2_array))
r_array = zeros(length(K_array),length(Ω_2_array))

counter = 1
pbar = ProgressBar(total=length(K_array)*length(Ω_2_array))
Ω_1, Ω_2 = 0, 0
for (i,_) in enumerate(K_array)
    for (j,_) in enumerate(Ω_2_array)
        if isodd(i)
            K, Ω_2 = K_array[i], Ω_2_array[j]
        else
            K, Ω_2 = K_array[i], Ω_2_array[length(Ω_2_array)-j+1]
        end
        ...
        for n=1:N-1
            ...
            data_array[counter,:] .= ϵ_n[n+1], Δnn_array[n], n, ω_0, Ω_1, ω_1, Ω_2, ω_2, K, ϵ_2, ϵ_1, N
            counter += 1
        end
...
heatmap(ϵ_1_array[2:end],ϵ_2_array[90:95],log.(matrix_color[2:end,90:95]'), xlab=L"\epsilon_1/\omega_0",ylab=L"\epsilon_2/\omega_0",title=L"N="*string(N)*L".\omega_0="*string(ω_0_exp)*"MHz.")
...
df_floquet = DataFrame(data_array, ["ϵ_n","Δnn","n","ω_0","Ω_1","ω_1","Ω_2","ω_2","K","ϵ_2","ϵ_1","N"]) 
