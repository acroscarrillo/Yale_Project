...
Ω_2_array = Vector( range(0, 3*ϵ_2*(1*K)/(4*g_n[1]), length=100) )
...
for Ω_2 in Ω_2_array
    for Ω_1 in Ω_1_array
        ...
        if Ω_2 == Ω_2_array[end]
            for i=1:N
                floquet_array[counter,:] .= ϵ_n[i]/(2*K), ϵ_2/(2*K), ϵ_1/(2*K), 1
                counter += 1
            end
        end
        update(pbar)
...
# df_floquet = DataFrame(data_array, ["ϵ_n","ω_0","Ω_1","ω_1","Ω_2","ω_2","K","ϵ_2","ϵ_1","N"]) 
df_floquet = DataFrame(vcat(floquet_array,h_eff_array), ["ΔE_n","ϵ_2","ϵ_1","floquet"]) 

CSV.write("data/floquet_crossings_epsilon_1.csv", df_floquet)
