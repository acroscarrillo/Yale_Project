...
@gif for ϵ_2 in ϵ_2_array
    ΔE_plots = zeros(length(ϵ_1_array), length(n_levels))
    for n in n_levels
        n = Int(n)
        ΔE_plots[:,n] = filter(row ->  row.ϵ_2 == ϵ_2 && row.n == n, df).ΔE_n
    end
    scatter(ϵ_1_array, ΔE_plots,markersize=1,markerstrokewidth=0,title="H_eff, N="*string(N[1])*", n_levels="*string(maximum(n_levels)),xlabel=L"\epsilon_1/K",ylabel=L"(E_n-E_0)/K",legend=false,ylim=(-5, 150))
